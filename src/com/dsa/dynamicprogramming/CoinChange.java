...
public class CoinChange {
    ...
    public static int coinChange(int[] coins, int amount) {
       ...
       for(int i=0; i<amount+1; i++){
           coinChange[i] = amount + 1;
       }

       for(int i=1; i<=amount; i++){ // 0,1,2,3,4,5
            for(int j=0; j<coins.length; j++){ // coins {1,2,3}
               if(coins[j]-i == 0){
                   coinChange[i] = 1;
               }else if(coins[j] > i){
                //Do nothing
               }else{
                   coinChange[i] = Math.min ( 1+ coinChange[i - coins[j]], coinChange[i] );
               }
            }
       }
